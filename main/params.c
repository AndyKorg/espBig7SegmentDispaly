/*
 * ќпределенные переменные и их сохранение и чтение
 */
#include <stdlib.h>
#include "sdkconfig.h"
#include "params.h"
...
/*
 * ¬озварщает значение переменной.
 * NULL если не найдена переменна€
 */
char* putsValue(char* toStr, char* varName, size_t *lenVal){
  ...
  if (value){
    ...
    *lenVal += strlen((char*) value);		//обща€ длина добавленых строк
    if (needFree){
      free(value);
    }
  }
